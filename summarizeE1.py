...
if  __name__ =='__main__':
    ...
    TotalTPS = quorum*batchsize*1000/Totalavglatency
    with open("./resultE1.txt", 'a') as dfile:
        p = time.strftime("%Y-%m-%d-%H%m%s", time.localtime()) + " Pando(" + str(config_data['committeeSizes']) + "): (TotalLatency," + str(round(Totalavglatency,2)) + " ms)" + "(TotalTPS," + str(round(TotalTPS,2)) + " tx/s)\n"
        print(p)
        dfile.write(p)
    dfile.close()
