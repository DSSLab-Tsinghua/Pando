...
Here, [N] is total number of replicas in the network, and [F] is the maximum Byzantine failures that the network could tolerate, which equals to (N - 1) / 3.  
...
The argument $committeeSizes$ need to be modified based on the experiment setups. For example, when we evaluate scenario Pando(0.8), we need to modify the $committeeSizes$ to $0.8$ in ```pando/etc/conf.json```. Also set $batchSize$ as $5000$.
...
Due to network bandwidth congestion, the execution could appear errors and get failed. When failure happends, just restart the experiement before kill all Pando process:
