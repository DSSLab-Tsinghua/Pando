...
func SetHomeDir() {
	...
	if err != nil {
		log.Fatalf("Cannot find absolute path of executable")
	}
...
func Store_share(share []byte, id int64) {
	...
	if !IsExist(path) {
		err := CreateDir(path)
		if err != nil {
			log.Fatalf("[Store Share Error] create file error! %v", err)
		}
	...
	if err != nil {
		log.Fatalf("[Store Share Error] write share error!%v", err)
		return
	}
...
func Store_key_dealer(vk, sk []uint64, n int64) {
	...
	for id := 0; id < int(n); id++ {
		...
		if !IsExist(path) {
			err := CreateDir(path)
			if err != nil {
				log.Fatalf("[KeyGen Error] create file error!%v", err)
			}
			fmt.Println("creating " + path)
		}
		...
		if err != nil {
			log.Fatalf("[Store_key_dealer Error] write vk error!%v", err)
			return
		}
		err = ioutil.WriteFile(path+skfileName, skb, 0644)
		if err != nil {
			log.Fatalf("[Store_key_dealer Error] write sk error!%v", err)
			return
		}
	}
}
...
func Store_vk_user(vk []byte, n int64) {
	...
	for id := 0; id < int(n); id++ {
		...
		err := ioutil.WriteFile(path+vkfileName, vkb, 0644)
		if err != nil {
			log.Fatalf("[Store_vk_user Error] write vk error!%v", err)
			return
		}
	}
}
...
func LoadvkFromFiles(id int64) (vk_x, vk_y []byte) {
	...
	if err != nil {
		p := fmt.Sprintf("[Loadvk Error] open vk file error!%v", err)
		logging.PrintLog(true, logging.ErrorLog, p)
		return nil, nil
	}
...
// *下载用户id的vk //*/
func PandoLoadvkFromFiles(id int64) []byte {
