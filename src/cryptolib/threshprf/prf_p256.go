...
func Prepare_key(vk, sk []uint64, n int64) ([][]byte, [][]byte, [][]byte) {

	var vkb, skb, vkx, vky []byte
	vkb = make([]byte, 64)
	skb = make([]byte, 32)
	var sk64, vk_x, vk_y [4]uint64

	var vkx_list, vky_list, skb_list [][]byte
	...
	for id := 0; id < int(n); id++ {
		...
		vkx = vkb[0:32]
		vky = vkb[32:64]
		vkx_list = append(vkx_list, vkx)
		vky_list = append(vky_list, vky)
		...
		skb_list = append(skb_list, skb)
	}

	return vkx_list, vky_list, skb_list
}

func Sign_dealer(vkx_list [][]byte, vky_list [][]byte, skb_list [][]byte, n int64, msg []byte) [][]byte {
	//250 bytes length
	// C := []byte("abcdefghiaerfvaghsthsthsrgerartargqretveqatvgqareAERGAEVFARsgiuoguiotdtyeyeeessse5362g35462b4tfwfsssabcdefghiaerfvgqretveqatvgqareAERGAEVFARsfwfssseAERGAEVFARsfwfsssabcdefghiaerfvgqretveqatvgqareAERGAEVFARsfwfsssabcdefghiaerfvgqretveqatvgqaresssefAee")
	C := msg
	fmt.Printf("Content len is %v bytes\n", len(C))
	//fmt.Printf("Content is %v",C)
	var share_list [][]byte

	for id := 0; id < int(n); id++ {
		share_list = append(share_list, Compute_share(C, skb_list[id], vkx_list[id], vky_list[id]))
	}

	return share_list
}

func Verify_dealer(vkx_list [][]byte, vky_list [][]byte, share_list [][]byte, n int64, msg []byte) {
	// C := []byte("abcdefghiaerfvaghsthsthsrgerartargqretveqatvgqareAERGAEVFARsgiuoguiotdtyeyeeessse5362g35462b4tfwfsssabcdefghiaerfvgqretveqatvgqareAERGAEVFARsfwfssseAERGAEVFARsfwfsssabcdefghiaerfvgqretveqatvgqareAERGAEVFARsfwfsssabcdefghiaerfvgqretveqatvgqaresssefAee")
	C := msg

	for id := 0; id < int(n); id++ {
		if Verify_share(C, vkx_list[id], vky_list[id], share_list[id]) == true {
			//fmt.Printf("\n share is true.")
			continue
		} else {
			fmt.Printf("\n share is false.")
...
func Combine_dealer(idarr []int64, k int64, share_list [][]byte) (prf []byte) {

	result := Compute_prf_from_shares(idarr, k, share_list)
	//fmt.Printf("result is %v",result)	//verify share and verify prf
	return result
}
...
func Init_key_dealer(n, k int64) {
	...
	log.Printf("Keygen Latency=%v ms.", t2-t1)

	Vkx_list, Vky_list, Skb_list := Prepare_key(vk, sk, int64(n))
	msg := make([]byte, 20<<20)                     //
	log.Printf("msg len = %v MB", len(msg)/1000000) //

	t11 := utils.MakeTimestamp()
	Share_list := Sign_dealer(Vkx_list, Vky_list, Skb_list, int64(n), msg)
	t22 := utils.MakeTimestamp()
	log.Printf("Sign Latency=%v ms.", t22-t11)
	//log.Printf("Share_list looks like: %v.", Share_list)

	t111 := utils.MakeTimestamp()
	Verify_dealer(Vkx_list, Vky_list, Share_list, int64(n), msg)
	t222 := utils.MakeTimestamp()
	log.Printf("Verify Latency=%v ms.", (t222-t111)/int64(n)*int64(n-k))
	...
	t1111 := utils.MakeTimestamp()
	prf := Combine_dealer(idarr, int64(k), Share_list)
	t2222 := utils.MakeTimestamp()
	log.Printf("Combine %v Latency=%v ms.", prf, t2222-t1111)

	// t11111 := utils.MakeTimestamp()
	// Verify_prf(prf)
	// t22222 := utils.MakeTimestamp()
	// log.Printf("Combine Latency=%v ms.", t22222-t11111)
