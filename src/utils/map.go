...
func (s *IntIntMap) Increment(key int) {
	s.Lock()
	defer s.Unlock()
	s.m[key] += 1
}
...
func (s *Int64IntMap) Init() {
	s.Lock()
	defer s.Unlock()
	s.m = make(map[int64]int)
}

func (s *Int64IntMap) Get(key int64) (int, bool) {
	s.RLock()
	defer s.RUnlock()
	_, exist := s.m[key]
	if exist {
...
func (s *Int64IntMap) GetAll() map[int64]int {
	return s.m
}

func (s *Int64IntMap) Insert(key int64, value int) {
	s.Lock()
	defer s.Unlock()
	s.m[key] = value
}

func (s *Int64IntMap) Set(value map[int64]int) {
	s.Lock()
	defer s.Unlock()
	s.m = value
}

func (s *Int64IntMap) Delete(key int64) {
	s.Lock()
	defer s.Unlock()
	_, exist := s.m[key]
	if exist {
...
func (s *Int64IntMap) Increment(key int64) int {
	s.Lock()
	defer s.Unlock()
	v, _ := s.m[key]
	v = v + 1
